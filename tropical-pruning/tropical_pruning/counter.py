...
def create_winner_counter(
    ...
    """
    Factory function to create the appropriate WinnerCounter for a model.

    Automatically detects whether the model contains Conv2d layers and selects:
    - ConvWinnerCounter for models with Conv2d layers
    - WinnerCounter for models with only Linear layers

    Args:
        model: The neural network model to analyze.
        layers: List of layer names to track. If None, auto-detects.
        track_margin: Whether to track winner margins.
        device: Device for computation. If None, uses model's device.
        include_linear: For conv models, whether to also track Linear layers.

    Returns:
        Appropriate WinnerCounter instance.
    ...
    if has_conv:
        ...
        from tropical_pruning.conv_counter import ConvWinnerCounter
        return ConvWinnerCounter(
            model=model,
            layers=layers,
